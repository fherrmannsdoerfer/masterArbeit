...
used as landmarks, because their position in the original image must be the same. The task is to find a transformation that maps corresponding beads on each
other.\newline
...
filters it is possible to capture pictures containing light emited from only one
kind of fluorophore.\newline
...
Beads can also be manually selecte or deleted, if the user wishes to do so. After the transformation the user can use the implemented tool for colocalization detection or save the transformed images and process them with the tool of his or her choice.\newline
...
\end{align}
The third contribution results from the fact that the fluorophors are not directly attached to the sample of interest, since the antibodies used for staining are inbetween. The upper bound of this error can be estimated under the assumption that the target of the antibodies is much smaller than the antibodies, so that there is no occlusion in the projection. This assumption does not hold in reality. Structures like cell membranes are certainly larger than the antibodies, but the assumption gives an upper bound. 
\begin{align}
...
Global colocalization has the drawback that there is just one value for the whole image, not yielding any information about where colocalization is localised. If there are regions in the image that show a lot of colocalization and other regions without colocalization the same colocalization coefficient might be achieved as if one channel is distributed randomly.\newline
For local colocalization analysis the algorithm from \cite{coloc} is used. It was further developed to gain a speed boost and runs approximatly 40 times faster now. This was achieved by using scipys (\cite{scipy}) ckdtree function, which is a k-d tree implemented in C.\newline
...
Figures \ref{radius50} and \ref{radius100} show the results from the colocalization calculation for both the red and green channel from \ref{cropedOrig}. The difference between \ref{radius50} and \ref{radius100} is that the maximal distance is larger for the later. This example shows that the larger the maximal distance the more colocalization is detected. The choice for the maximal distance depends strongly on the sample.
...
\end{minipage}\\
\begin{minipage}[t]{0.99\textwidth}
\subfloat[]{\includegraphics[width = 0.485\textwidth]{pictures/colocalization/cropedCh0.png}}\hfill
\subfloat[]{\includegraphics[width = 0.485\textwidth]{pictures/colocalization/cropedCh1.png}}
	\caption{Colocalization for the red and green channel with a maximal radius $R=50$ nm}
	\label{radius50}	
\end{minipage}\\
\begin{minipage}[t]{0.99\textwidth}
\subfloat[]{\includegraphics[width = 0.485\textwidth]{pictures/colocalization/cropedCh0100.png}}\hfill
\subfloat[]{\includegraphics[width = 0.485\textwidth]{pictures/colocalization/cropedCh1100.png}}
